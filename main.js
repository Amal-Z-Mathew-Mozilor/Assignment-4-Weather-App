...
async function getCurrentLocation() {
  ...
  try {
    ...
    const result = dashboard.addCity("Current Location", latitude, longitude);
...
async function handleSearch() {
  ...
  try {
    const { name, latitude, longitude, country } = await getCoordinates(value);

    const exists = dashboard
      .getCities()
    ...
    const result = dashboard.addCity(name, latitude, longitude, country);

    if (!result.success) {
      if (result.reason === "limit") {
        showToast("Maximum 8 cities reached — remove one first.", "error");
  ...
  showNetworkOverlay();
   ...
   showToast(`City "${value}" not found`, "error");
}
  }
}
...
historyBox.addEventListener("click", (e) => {
  ...
  if (lat && lon) {
    ...
    const result = dashboard.addCity(name, +lat, +lon);
...
function render() {
  ...
  document.querySelectorAll(".retry-btn").forEach((btn) => {
    btn.onclick = () => {
      const city = dashboard
        .getCities()
        .find((c) => c.name === btn.dataset.city);
